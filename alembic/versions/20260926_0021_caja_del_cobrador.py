...
def upgrade():
    op.create_table(
        ...
        # El valor es siempre positivo: el sentido lo pone el tipo. Un signo
        # guardado dentro del numero se equivoca en silencio y en un cuadre de
        # caja eso no se nota -- simplemente le cuadra a quien no deberia.
    ...
    op.create_index("ix_movimientos_caja_empresa_id", "movimientos_caja", ["empresa_id"])
    op.create_index("ix_movimientos_caja_usuario_id", "movimientos_caja", ["usuario_id"])
    ...
    op.create_index("ix_movimientos_empresa_usuario_fecha", "movimientos_caja",
                    ["empresa_id", "usuario_id", "fecha"])
    op.create_index("ix_movimientos_empresa_fecha", "movimientos_caja",
                    ["empresa_id", "fecha"])
    ...
    # restringido podria leer entera.
    ...
    op.create_index("ix_prestamos_desembolsado_por_id", "prestamos",
                    ["desembolsado_por_id"])
    op.create_index("ix_prestamos_fecha_desembolso", "prestamos", ["fecha_desembolso"])
    ...
    if op.get_bind().dialect.name == "postgresql":
        op.create_foreign_key("fk_prestamo_desembolsado_por", "prestamos", "usuarios",
                              ["desembolsado_por_id"], ["id"], ondelete="SET NULL")
...
def downgrade():
    if op.get_bind().dialect.name == "postgresql":
        op.drop_constraint("fk_prestamo_desembolsado_por", "prestamos",
                           type_="foreignkey")
