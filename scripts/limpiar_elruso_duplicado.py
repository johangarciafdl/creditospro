...
Nota: este script tambien se copia suelto a la raiz de otros equipos via
copiar_a_usb.bat/sync_archivos.bat, no siempre vive en scripts/ como aqui.
Por eso busca la raiz real del proyecto en vez de asumir una profundidad fija.
"""
import sys
import os
...
def _find_root(start: Path) -> Path:
    for candidate in [start, *start.parents]:
        if (candidate / "app" / "database.py").exists():
            return candidate
    return start
...
if env_file.exists():
    ...
    print(f"[OK] Variables cargadas desde: {env_file}")
else:
