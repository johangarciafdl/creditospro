...
async def auth_middleware(request: Request, call_next):
    ...
    publicas = ["/auth/", "/static/", "/uploads/", "/setup", "/favicon.ico", "/api/", "/debug-hash"]
...
@app.get("/debug-hash")
async def debug_hash_global(password: str = "admin123"):
    import bcrypt
    salt = bcrypt.gensalt()
    hashed = bcrypt.hashpw(password.encode('utf-8'), salt).decode('utf-8')
    ok = bcrypt.checkpw(password.encode('utf-8'), hashed.encode('utf-8'))
    return {"hash": hashed, "ok": ok}


@app.exception_handler(FastAPIHTTPException)
async def http_exception_handler(request: Request, exc: FastAPIHTTPException):
    if exc.status_code in (303, 307) and exc.headers and exc.headers.get("Location"):
